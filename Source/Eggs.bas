...
160 GOSUB 640 : GOSUB 680 : GOSUB 790 : GOSUB 820
170 REPEAT
...
230 KEY% = INKEY(1)
240 IF KEY% = 97 AND X% - 2 > 0 THEN X% = X% - 1
250 IF KEY% = 100 AND X% + 2 < 19 THEN X% = X% + 1
260 IF KEY% = 119 AND BASKET% <> 0 THEN GOSUB 740
...
520 GOSUB 680 : GOSUB 790
530 RETURN
540 REM Missed egg
550 SOUND 0,-10,22,18 
560 IF C% <> 3 THEN LIVES% = LIVES% - 1
570 GOSUB 640
580 IF BASKET% = 10 THEN BASKET% = 0 : GOSUB 790
590 RETURN
600 REM Level up
...
700 IF SCORE% > BONUS% THEN GOSUB 860
710 PRINT TAB(1,2)CHR$(17)+CHR$(7)LEFT$(STR$(SCORE%),8)
720 PRINT TAB(11,2)CHR$(17)+CHR$(7)LEFT$(STR$(HIGH%),8)
730 RETURN
740 REM Empty basket
750 SCORE% = SCORE% + BASKET% * 10
760 BASKET% = 0
770 GOSUB 680 : GOSUB 790
780 RETURN
790 REM Basket status
800 IF BASKET% > 0 THEN PRINT TAB(9,30)STR$(BASKET% * 10) ELSE PRINT TAB(9,30)"   "
810 RETURN
820 REM Level status
830 PRINT TAB(17,30)"  "
840 PRINT TAB(17,30)CHR$(17)+CHR$(3)LEFT$(STR$(LEVEL%),2)
850 RETURN
860 REM 1Up
870 IF LIVES% < 3 THEN LIVES% = LIVES% + 1 ELSE SCORE% = SCORE% + 150
880 BONUS% = BONUS% + 10000
890 SOUND 1,-15,60,10
900 GOSUB 640 : GOSUB 680
910 RETURN
920 REM Title screen
930 MODE 7
940 VDU 23,1,0;0;0;0;
950 PRINT TAB(10,8)CHR$(141)CHR$(131)"Eggs"CHR$(135)"'N'"CHR$(131)"Basket"
960 PRINT TAB(10,9)CHR$(141)CHR$(135)"Eggs 'N' Basket"
970 PRINT TAB(9,13)CHR$(130)"Keys:"
980 PRINT TAB(16,13)CHR$(134)"A = Move left."
990 PRINT TAB(16,14)CHR$(134)"D = Move right."
1000 PRINT TAB(16,15)CHR$(134)"W = Empty basket."
1010 PRINT TAB(12,22)CHR$(135)CHR$(136)"Press SPACE"
1020 REPEAT
1030 KEY% = INKEY(1)
1040 UNTIL KEY% = 32
1050 RETURN